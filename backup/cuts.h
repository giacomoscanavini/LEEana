...
namespace LEEana{
  ...
  bool is_NCpio_bdt_tight(TaggerInfo& tagger_info, KineInfo& kine);
...
double LEEana::get_kine_var(KineInfo& kine, EvalInfo& eval, PFevalInfo& pfeval, TaggerInfo& tagger, bool flag_data , TString var_name){
  ...
  if (var_name == "kine_reco_Enu"){
    ...
    if (kine.kine_pio_mass >0){
      ...
      if (flag_data) { return kine.kine_pio_mass * em_charge_scale;
      }else{ return kine.kine_pio_mass;
      }
    }else{ return kine.kine_pio_mass;
    }
    //  }else if (var_name == "pi0_mass"){
    ...
    double pi0_energy_tot = pi0_mass + pi0_energy;
    return sqrt(pi0_energy_tot*pi0_energy_tot - pi0_mass*pi0_mass);
...
int LEEana::get_xs_signal_no(int cut_file, std::map<TString, int>& map_cut_xs_bin, EvalInfo& eval, PFevalInfo& pfeval, TaggerInfo& tagger, KineInfo& kine){
  for (auto it = map_cut_xs_bin.begin(); it != map_cut_xs_bin.end(); it++){
	...
	std::cout << "get_xs_signal_no: no cut found!" << std::endl;
      ...
      if (cut_name == "numuCC.inside.Emuon.le.226.gt.106"){
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && Emuon<=226 && Emuon>106) return number;
  ...
  std::cout << "get_xs_signal_no: no cut found!" << std::endl;
    ...
    //very coarse angle binning
      ...
      if (cut_name == "NCpi0BDT.inside.Enu.le.540.gt.200"){ // recommended range: 200 - 540
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=540 && eval.truth_nuEnergy>200) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.705.gt.540"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=705 && eval.truth_nuEnergy>540) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.805.gt.705"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=805 && eval.truth_nuEnergy>705) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.920.gt.805"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=920 && eval.truth_nuEnergy>805) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.1050.gt.920"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=1050 && eval.truth_nuEnergy>920) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.1200.gt.1050"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=1200 && eval.truth_nuEnergy>1050) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.1375.gt.1200"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=1375 && eval.truth_nuEnergy>1200) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.1570.gt.1375"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=1570 && eval.truth_nuEnergy>1375) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.2050.gt.1570"){
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=2050 && eval.truth_nuEnergy>1570) return number;
            }else if (cut_name == "NCpi0BDT.inside.Enu.le.4000.gt.2050"){ // recommended range: 2050 - 4000
        if (eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy>2050 && eval.truth_nuEnergy<=4000) return number;
            }else{
        std::cout << "get_xs_signal_no: no cut found!" << std::endl;
      }
    }
      ...
      if (cut_name == "numuCC.inside.Enu.le.5000.gt.200"){ 
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && eval.truth_nuEnergy<=5000 && eval.truth_nuEnergy>200) return number;
            }else{
        std::cout << "get_xs_signal_no: no cut found!" << std::endl;
      }
    }

    else if (cut_file == 19){ // pmuon
      if (cut_name == "numuCC.inside.Pmuon.le.180.gt.0"){
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && pmuon>=0.00 && pmuon<180.00) return number;
            }else if (cut_name == "numuCC.inside.Pmuon.le.300.gt.180"){
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && pmuon>=180.00 && pmuon<300.00) return number;
            }else if (cut_name == "numuCC.inside.Pmuon.le.450.gt.300"){
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && pmuon>=300.00 && pmuon<450.00) return number;
            }else if (cut_name == "numuCC.inside.Pmuon.le.770.gt.450"){
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && pmuon>=450.00 && pmuon<770.00) return number;
            }else if (cut_name == "numuCC.inside.Pmuon.le.1280.gt.770"){
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && pmuon>=770.00 && pmuon<1280.00) return number;
            }else if (cut_name == "numuCC.inside.Pmuon.le.2500.gt.1280"){
        if (eval.truth_nuPdg==14 && eval.truth_isCC==1 && eval.truth_vtxInside==1 && pmuon>=1280.00 && pmuon<2500.00) return number;
            }else{
        std::cout << "get_xs_signal_no: no cut found!" << std::endl;
      }
    }
...
bool LEEana::get_cut_pass(TString ch_name, TString add_cut, bool flag_data, EvalInfo& eval, PFevalInfo& pfeval, TaggerInfo& tagger, KineInfo& kine){
  ...
  if(eval.match_completeness_energy>0.1*eval.truth_energyInside && eval.truth_isCC==0 && eval.truth_vtxInside==1 && pfeval.truth_NprimPio>0 && eval.truth_nuEnergy<=4000 && eval.truth_nuEnergy > 200) map_cuts_flag["Xs_Enu_NCpi0BDTinFV"] = true;
  ...
  bool flag_ncpio_bdt_tight = is_NCpio_bdt_tight(tagger, kine);
 ...
 // cut-based numuCC FC/PC 2 obs channels   
  ...
  // Generic Neutrino Selection
        ...
        || ch_name == "generic3_bnb" || ch_name == "generic3_ext"  || ch_name == "generic3_dirt"  || ch_name == "generic3_overlay"
        || ch_name == "generic4_bnb" || ch_name == "generic4_ext"  || ch_name == "generic4_dirt"  || ch_name == "generic4_overlay"
    ){
    if (flag_generic) return true;
    else return false;
  }else if (ch_name == "generic_FC_overlay" || ch_name == "generic_FC_ext" || ch_name =="generic_FC_dirt" || ch_name == "generic_FC_bnb"){
    if (flag_generic && flag_FC) return true;
    else return false;
  }else if (ch_name == "generic_PC_overlay" || ch_name == "generic_PC_ext" || ch_name =="generic_PC_dirt" || ch_name == "generic_PC_bnb"){
    if (flag_generic && (!flag_FC)) return true;
    else return false;
  ...
  ////////////////////////////////////////////////////////
       ...
       || ch_name == "NCpi0BDT1_FC_bnb" || ch_name == "NCpi0BDT1_FC_ext" || ch_name == "NCpi0BDT1_FC_dirt"  || ch_name == "NCpi0BDT1_FC_overlay" || ch_name == "BG_NCpi0BDT1_FC_ext" || ch_name == "BG_NCpi0BDT1_FC_dirt"
       || ch_name == "NCpi0BDT2_FC_bnb" || ch_name == "NCpi0BDT2_FC_ext" || ch_name == "NCpi0BDT2_FC_dirt"  || ch_name == "NCpi0BDT2_FC_overlay" || ch_name == "BG_NCpi0BDT2_FC_ext" || ch_name == "BG_NCpi0BDT2_FC_dirt"
       || ch_name == "NCpi0BDT3_FC_bnb" || ch_name == "NCpi0BDT3_FC_ext" || ch_name == "NCpi0BDT3_FC_dirt"  || ch_name == "NCpi0BDT3_FC_overlay" || ch_name == "BG_NCpi0BDT3_FC_ext" || ch_name == "BG_NCpi0BDT3_FC_dirt"
       ){
    if (flag_ncpio_bdt && flag_generic && flag_filter_showers && flag_FC) return true;
    else return false;
  }else if (ch_name == "NCpi0BDT_FC_0p_bnb" || ch_name == "NCpi0BDT_FC_0p_ext" || ch_name == "NCpi0BDT_FC_0p_dirt"  || ch_name == "NCpi0BDT_FC_0p_overlay"
  ...
  }else if (ch_name == "NCpi0BDT_FC_Np_bnb" || ch_name == "NCpi0BDT_FC_Np_ext" || ch_name == "NCpi0BDT_FC_Np_dirt"  || ch_name == "NCpi0BDT_FC_Np_overlay"
  ...
  ////////////////////////////////////////////////////////
       ...
       || ch_name == "NCpi0BDT1_PC_bnb" || ch_name == "NCpi0BDT1_PC_ext" || ch_name == "NCpi0BDT1_PC_dirt"  || ch_name == "NCpi0BDT1_PC_overlay" || ch_name == "BG_NCpi0BDT1_PC_ext" || ch_name == "BG_NCpi0BDT1_PC_dirt"
       || ch_name == "NCpi0BDT2_PC_bnb" || ch_name == "NCpi0BDT2_PC_ext" || ch_name == "NCpi0BDT2_PC_dirt"  || ch_name == "NCpi0BDT2_PC_overlay" || ch_name == "BG_NCpi0BDT2_PC_ext" || ch_name == "BG_NCpi0BDT2_PC_dirt"
       || ch_name == "NCpi0BDT3_PC_bnb" || ch_name == "NCpi0BDT3_PC_ext" || ch_name == "NCpi0BDT3_PC_dirt"  || ch_name == "NCpi0BDT3_PC_overlay" || ch_name == "BG_NCpi0BDT3_PC_ext" || ch_name == "BG_NCpi0BDT3_PC_dirt"
       ){
    if (flag_ncpio_bdt && flag_generic && flag_filter_showers && (!flag_FC)) return true;
    else return false;
  }else if (ch_name == "NCpi0BDT_PC_0p_bnb" || ch_name == "NCpi0BDT_PC_0p_ext" || ch_name == "NCpi0BDT_PC_0p_dirt"  || ch_name == "NCpi0BDT_PC_0p_overlay"
  ...
  }else if (ch_name == "NCpi0BDT_PC_Np_bnb" || ch_name == "NCpi0BDT_PC_Np_ext" || ch_name == "NCpi0BDT_PC_Np_dirt"  || ch_name == "NCpi0BDT_PC_Np_overlay"
 ...
 // nueCC 3 variables: n_trakcs, n_showers, gap_n_bad, FC/PC x3 = 6 channels; 4 additional channels 
    ...
    return false;
       ...
       || ch_name == "NCpi0BDT1_signal_Enu_FC_overlay" || ch_name == "NCpi0BDT1_signal_Enu_PC_overlay" || ch_name == "NCpi0BDT1_background_Enu_FC_overlay" || ch_name == "NCpi0BDT1_background_Enu_PC_overlay"
       || ch_name == "NCpi0BDT2_signal_Enu_FC_overlay" || ch_name == "NCpi0BDT2_signal_Enu_PC_overlay" || ch_name == "NCpi0BDT2_background_Enu_FC_overlay" || ch_name == "NCpi0BDT2_background_Enu_PC_overlay"
       || ch_name == "NCpi0BDT_signal_Enu_overlay" || ch_name == "NCpi0BDT_background_Enu_overlay"
      ){
    if (ch_name == "NCpi0BDT_signal_Enu_FC_overlay" || ch_name == "NCpi0BDT1_signal_Enu_FC_overlay" || ch_name == "NCpi0BDT2_signal_Enu_FC_overlay"){
      if (flag_ncpio_bdt && flag_generic && flag_filter_showers && flag_FC && map_cuts_flag["Xs_Enu_NCpi0BDTinFV"]) return true;
    }else if (ch_name == "NCpi0BDT_signal_Enu_PC_overlay" || ch_name == "NCpi0BDT1_signal_Enu_PC_overlay" || ch_name == "NCpi0BDT2_signal_Enu_PC_overlay" ){
      if (flag_ncpio_bdt && flag_generic && flag_filter_showers && (!flag_FC) && map_cuts_flag["Xs_Enu_NCpi0BDTinFV"]) return true;
    }else if (ch_name == "NCpi0BDT_background_Enu_FC_overlay" || ch_name == "NCpi0BDT1_background_Enu_FC_overlay" || ch_name == "NCpi0BDT2_background_Enu_FC_overlay"){
      if (flag_ncpio_bdt && flag_generic && flag_filter_showers && flag_FC && (!map_cuts_flag["Xs_Enu_NCpi0BDTinFV"])) return true;
    }else if (ch_name == "NCpi0BDT_background_Enu_PC_overlay" || ch_name == "NCpi0BDT1_background_Enu_PC_overlay" || ch_name == "NCpi0BDT2_background_Enu_PC_overlay"){
      if (flag_ncpio_bdt && flag_generic && flag_filter_showers && (!flag_FC) && (!map_cuts_flag["Xs_Enu_NCpi0BDTinFV"])) return true;
    }else if (ch_name == "NCpi0BDT_signal_Enu_overlay"){
      if (flag_ncpio_bdt && flag_generic && flag_filter_showers && map_cuts_flag["Xs_Enu_NCpi0BDTinFV"]) return true;
    }else if (ch_name == "NCpi0BDT_background_Enu_overlay"){
      if (flag_ncpio_bdt && flag_generic && flag_filter_showers && (!map_cuts_flag["Xs_Enu_NCpi0BDTinFV"])) return true;
    }
    return false;
...
bool LEEana::is_NCpio_bdt_tight(TaggerInfo& tagger_info, KineInfo& kine){
  bool flag = false;
