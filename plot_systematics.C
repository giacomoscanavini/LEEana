#include "../wcp-uboone-bdt/src/draw.icc"
...
{  
  ...
  // MODIFY THIS PART BASED ON THE PLOT
  const int num_ch = 2;
  int nbins_ch[num_ch] = {29,29};
  double bin_start[num_ch] = {0.,0.};
  double bin_end[num_ch] = {300.,300.};
  double factor[num_ch] = {1.,1.};

  for(int i=0; i<num_ch; i++){
    factor[i] = bin_end[i] - bin_start[i];
    factor[i] = factor[i]/nbins_ch[i];
  }
  char x_label[] = "#pi^{0} Mass [Bin Num.]";
  ...
  for(int ich=0; ich<num_ch; ich++) {
    ...
    for(int ibin=0; ibin<nbins_ch[ich]; ibin++) {
      line_str++;
      //if(ibin == 0) axis_label_str[line_str-1] = TString::Format("%.1lf", bin_start[ich] + ibin*factor[ich]);
      //else if((ibin)%5 == 0) axis_label_str[line_str-1] = TString::Format("%.1lf", bin_start[ich] + ibin*factor[ich]);
      if(ibin == 0) axis_label_str[line_str-1] = TString::Format("%d", ibin);
      else if((ibin)%5 == 0) axis_label_str[line_str-1] = TString::Format("%d", ibin);
      else axis_label_str[line_str-1] ="";
    }
    line_str++;
  }// ich
  ...
  */
  ...
  TString pt_str_ch[num_ch] = {"FC 0p", "FC Np"};
  double pt_str_angle[num_ch] = {0,0};
  for(int idx=0; idx<num_ch; idx++) {
    int line_eff = 0;
  ...
  func_xy_title(h2_correlation_flux, x_label, x_label);
  ...
  func_xy_title(h2_correlation_Xs, x_label, x_label);
  ...
  func_xy_title(h2_correlation_detector, x_label, x_label);
  ...
  for(auto it=matrix_absolute_detector_sub_cov_newworld.begin(); it!=matrix_absolute_detector_sub_cov_newworld.end(); it++) {
    ...
    func_xy_title(h2_correlation_detector_sub[idx], x_label, x_label);
  ...
  func_xy_title(h2_correlation_mc_stat, x_label, x_label);
  ...
  func_xy_title(h2_correlation_additional, x_label, x_label);
  ...
  func_xy_title(h2_correlation_total, x_label, x_label);
  ...
  func_xy_title(h2_relerr_flux_Xs, x_label, "Relative uncertainty");
  ...
  func_xy_title(h2_relerr_detector, x_label, "Relative error");
  ...
  func_xy_title(h2_relerr_total, x_label, "Relative error");
  ...
  TH2D *h2_basic_fraction = new TH2D(roostr, "", rows, 0, rows, 100, 0, 100);
  for(int ibin=1; ibin<=rows; ibin++) {
  ...
  func_xy_title(h2_basic_fraction, x_label, "Syst. percentage");
  ...
  for(int idx=0; idx<num_ch; idx++) {
    pt_text_ch[idx]->Draw();
    pt_text_ch[idx]->SetY1(100);
    pt_text_ch[idx]->SetY2(100);
    int line_eff = 0;
    for(int jdx=0; jdx<idx; jdx++) line_eff += nbins_ch[jdx];
    pt_text_ch[idx]->SetX1(line_eff+4);
    pt_text_ch[idx]->SetX2(line_eff+4+1);
  ...
  TH2D *h2_basic_fraction_detector = new TH2D(roostr, "", rows, 0, rows, 100, 0, 100);
  for(int ibin=1; ibin<=rows; ibin++) {
  ...
  func_xy_title(h2_basic_fraction_detector, x_label, "Syst. percentage");
