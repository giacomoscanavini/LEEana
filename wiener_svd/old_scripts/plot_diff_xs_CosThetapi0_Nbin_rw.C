...
// main function
void plot_diff_xs_CosThetapi0_Nbin_rw(double XS_FACTOR, double NUCLEONS){
  double xbins[] = {-1.0, -0.62, -0.13, 0.20, 0.47, 0.67, 0.83, 0.95, 1.0, 1.38, 1.87, 2.2, 2.47, 2.67, 2.83, 2.95, 3.0}; //Bin separators 
  //double xbins[] = {0., 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.8, 1.0, 1.5, 1.6, 1.65, 1.7, 1.75, 1.8, 1.9, 2.0, 2.1, 2.3, 2.5, 3.}; //Bin separators 
  //std::cout << " Open data set binning! (0p, Np) " << std::endl;

  //double xbins[] = {-1.0, -0.62, -0.13, 0.20, 0.47, 0.67, 0.83, 0.95, 1.0}; // Bin separators 
  //double xbins[] = {-1.0, -0.62, -0.34, -0.13, 0.06, 0.20, 0.32, 0.42, 0.52, 0.6, 0.67, 0.73, 0.78, 0.83, 0.87, 0.91, 0.95, 0.975, 1.0}; // Bin separators
  ...
  XS_FACTOR = XS_FACTOR/NUCLEONS;  // Only if you want XS normalized per nucleon
  ...
  for (int i=0;i<n;i++) {
    for (int j=0;j<n;j++) {
      ...
      mcov_rw_cor(i,j) = hcov_rw_cor->GetBinContent(  i+1,j+1);
  ...
  for (int i=0;i<m;i++) {
    ...
    if (XS_FACTOR == 1.) std::cout << "CV = " << content << " [1e-36 cm^2 / 1 / Ar]" << std::endl;
    else if (XS_FACTOR == 100.) std::cout << "CV = " << content << " [1e-38 cm^2 / 1 / Ar]" << std::endl;
    else if (XS_FACTOR == 1000.) std::cout << "CV = " << content << " [1e-39 cm^2 / 1 / Ar]" << std::endl;
    else if (XS_FACTOR == 10000.) std::cout << "CV = " << content << " [1e-40 cm^2 / 1 / Ar]" << std::endl;

    else if (XS_FACTOR == 1./NUCLEONS) std::cout << "CV = " << content << " [1e-38 cm^2 / 1 / nucleon]" << std::endl;
    else if (XS_FACTOR == 100./NUCLEONS) std::cout << "CV = " << content << " [1e-38 cm^2 / 1 / nucleon]" << std::endl;
    else if (XS_FACTOR == 1000./NUCLEONS) std::cout << "CV = " << content << " [1e-39 cm^2 / 1 / nucleon]" << std::endl;
    else if (XS_FACTOR == 10000./NUCLEONS) std::cout << "CV = " << content << " [1e-40 cm^2 / 1 / nucleon]" << std::endl;
    ...
    double abs_syst = TMath::Sqrt( abs_mcstat*abs_mcstat + abs_add*abs_add + abs_flux*abs_flux + abs_det*abs_det + abs_xs*abs_xs + abs_rw*abs_rw + abs_rw_cor*abs_rw_cor);
    ...
    vec_err_rw_cor[i] = 100.*frac_rw_cor;
    ...
    herr_frac_rw_cor->SetBinContent(i+1,frac_rw_cor);
  ...
  std::cout << "err_rw_cor = ["; 
  for (int i=0; i<m; i++){
    if (i==0) std::cout << vec_err_rw_cor[i];
    else std::cout << ", " << vec_err_rw_cor[i];
  }
  std::cout << "]" << std::endl;
  ...
  herr_frac_rw->SetTitle("RW");
  herr_frac_rw_cor->SetTitle("RW cor");
  if(m == 1){
  ...
  herr_frac_rw->SetLineColor(41);
  herr_frac_rw_cor->SetLineColor(45);
  ...
  herr_frac_rw->SetLineWidth(2);
  herr_frac_rw_cor->SetLineWidth(2);
  ...
  herr_frac_rw->Draw("same");
  herr_frac_rw_cor->Draw("same");
  ...
  le2->AddEntry(herr_frac_rw,"RW","l");
  le2->AddEntry(herr_frac_rw_cor,"RW cor","l");
