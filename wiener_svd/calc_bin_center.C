void calc_bin_center(){
  auto file = TFile::Open("../processed_checkout_rootfiles/checkout_prodgenie_bnb_nu_overlay_run1.root");
...
//    cout << i << " " << truth_nuPdg << " " << truth_isCC << " " << truth_vtxInside << " " << truth_muonMomentum[3] << endl;
    ...
    if (truth_nuPdg==14 && truth_isCC==1 && truth_vtxInside==1) {
      ...
      if(index!=-1) {
        h1->Fill(index, truth_nuEnergy * weight_cv * weight_spline);
      ...
      /*
      // calculate bin center for Pmuon
      double KE_muon = truth_muonMomentum[3]*1000.-105.66; // MeV
      double pmuon   = TMath::Sqrt(pow(KE_muon,2) + 2*KE_muon*105.66); // MeV
      if (pmuon<=180 && pmuon>0) index = 0;
      else if (pmuon<=300 && pmuon>180) index = 1;
      else if (pmuon<=450 && pmuon>300) index = 2;
      else if (pmuon<=770 && pmuon>450) index = 3;
      else if (pmuon<=1280 && pmuon>770) index = 4;
      else if (pmuon<=2500 && pmuon>1280) index = 5;

      if(index!=-1) {
        h1->Fill(index, pmuon * weight_cv * weight_spline);
        h2->Fill(index, weight_cv * weight_spline);
      }
      */
