...
// main function
void plot_total_xs_Ppi0_1bin(){

  double xbins[] = {0., 2}; // Bin separators - GeV
  ...
  gr_tot->GetYaxis()->SetRangeUser(0., 0.03 * XS_FACTOR);
  gr_tot->GetXaxis()->SetLimits(0.,xbins[1]);  // along X
...
*/
  ...
  auto g_glee_2gXp = new TGraphAsymmErrors(1, x_glee_2gXp, y_glee_2gXp, ex_glee_2gXp, ex_glee_2gXp, ey_glee_2gXp, ey_glee_2gXp);
  g_glee_2gXp->SetLineColor(4);
  g_glee_2gXp->SetLineWidth(2);
  g_glee_2gXp->SetMarkerColor(4);
  g_glee_2gXp->SetMarkerStyle(20);
  g_glee_2gXp->Draw("Psame");
  ...
  g_NCpi0BDT_Enu_2gXp->SetLineColor(46);
  g_NCpi0BDT_Enu_2gXp->SetLineWidth(2);
  g_NCpi0BDT_Enu_2gXp->SetMarkerColor(46);
  g_NCpi0BDT_Enu_2gXp->SetMarkerStyle(20);
  //g_NCpi0BDT_Enu_2gXp->Draw("Psame");


  //auto lg = new TLegend(0.11,0.7,0.5,0.89);
  auto lg = new TLegend(0.55, 0.15, 0.89, 0.35);
  lg->AddEntry(gr_stat, "BNB (Stat.)","lpe");
  lg->AddEntry(gr_tot, "BNB (Total)","lpe");
  lg->AddEntry(line, Form("MC GENIE [#chi^{2}/ndf: %.1f/%d]",chi2_m3,nbins), "l");
