/*
  ...
  It works specifically for 4 reconstructed channels [FC, PC] (order must be kept in cov_input.txt)

  nbin_mom: 12 bins in reco space for momentum (overflow not considered)
  nbin_cos: 4 slices for costheta 
  
  To run use: 
    root -l --web=off 'convert_2D_wiener_2ch.C(1,12,4)'   (yes reweighting, nbin_mom, nbin_cos)
    root -l --web=off 'convert_2D_wiener_2ch.C(0,12,4)'   (no reweighting, nbin_mom, nbin_cos)
*/
void convert_2D_wiener_2ch(int RW=1, int nbin_mom=12, int nbin_cos=4){
  gStyle->SetOptStat(0);
  ...
  for (int i=0;i<nbin_cos;i++) {
    ...
    std::string histo_str_1 = "histo_" + std::to_string(FC0p_index + 0*2*nbin_cos);  // for i==0,1,2,3 --> 1  2  3  4   // changes with the number of costheta slices
    std::string histo_str_2 = "histo_" + std::to_string(FCNp_index + 0*2*nbin_cos);  // for i==0,1,2,3 --> 5  6  7  8 

    std::string histo_str_3 = "histo_" + std::to_string(FC0p_index + 1*2*nbin_cos);  // for i==0,1,2,3 --> 17 18 19 20  // changes with the number of costheta slices
    std::string histo_str_4 = "histo_" + std::to_string(FCNp_index + 1*2*nbin_cos);  // for i==0,1,2,3 --> 21 22 23 24

    std::string histo_str_5  = "histo_" + std::to_string(FC0p_index + 2*2*nbin_cos); // for i==0,1,2,3 --> 33 34 35 36  // changes with the number of costheta slices
    std::string histo_str_6 = "histo_" + std::to_string(FCNp_index + 2*2*nbin_cos); // for i==0,1,2,3 --> 37 38 39 40
    ...
    TH1F *histo_3_tmp       = (TH1F*)file1->Get(histo_str_3.c_str());   // MC bkg (dirt included) FC 0p
    TH1F *histo_4_tmp       = (TH1F*)file1->Get(histo_str_4.c_str());   // MC bkg (dirt included) FC Np
    TH1F *histo_5_tmp       = (TH1F*)file1->Get(histo_str_5.c_str());   // EXT FC 0p
    TH1F *histo_6_tmp       = (TH1F*)file1->Get(histo_str_6.c_str());   // EXT FC Np
    ...
    for (int j=0;j<nbin_mom;j++) {
      ...
      histo_bkg_1->SetBinContent(index, histo_3_tmp->GetBinContent(j+1));         // MC bkg (dirt included) FC 0p
      histo_bkg_2->SetBinContent(index, histo_4_tmp->GetBinContent(j+1));         // MC bkg (dirt included) FC Np
      histo_ext_1->SetBinContent(index, histo_5_tmp->GetBinContent(j+1));         // EXT FC 0p
      histo_ext_2->SetBinContent(index, histo_6_tmp->GetBinContent(j+1));        // EXT FC Np
  ...
  Int_t nbin_R = hdata_obsch_1->GetNbinsX() + hdata_obsch_2->GetNbinsX();
  std::cout << "nbin_R = " << nbin_R << " [nbin_cos * 2 ch * nbin_mom]" << std::endl;
  ...
  for (int i=0;i<nbin_cos;i++) {
    for (int j=0;j<nbin_mom;j++) {
      ...
      */
      ...
      std::cout << "Indexes = " << index1 << " " << index2 << " " << index3 << " " << index4 << " " << index5 << " " << index6 << std::endl;

      vec_nominal(index1) = histo_sig_1->GetBinContent(index0+1);
      vec_nominal(index2) = histo_sig_2->GetBinContent(index0+1);
      vec_nominal(index3) = histo_bkg_1->GetBinContent(index0+1);
      vec_nominal(index4) = histo_bkg_2->GetBinContent(index0+1);
      vec_nominal(index5) = histo_ext_1->GetBinContent(index0+1);
      vec_nominal(index6) = histo_ext_2->GetBinContent(index0+1);
    }
  }
