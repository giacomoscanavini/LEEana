/*
	...
	It's meant to be used for the extraction of the single bin total cross section (in my case NCpi0 Xp)
	...
	It works specifically for a single channel [combined] (order must be kept in cov_input.txt)
	
	To run use: s
		root -l --web=off 'convert_single_wiener.C(1)'		(yes reweighting)
		root -l --web=off 'convert_single_wiener.C(0)'		(no reweighting)
*/
void convert_single_wiener(int RW=1){
	gStyle->SetOptStat(0);
	...
	TH1F *histo_bkg_1 = (TH1F*)file1->Get("histo_2"); 		// MC bkg (nu_overlay + dirt) 
	TH1F *histo_ext_1 = (TH1F*)file1->Get("histo_3"); 		// EXT 

	TH1F *hmc_obsch_1 = (TH1F*)file1->Get("hmc_obsch_1");   	// Total MC

	// Number of bins in reco space
	Int_t nbin_R = hdata_obsch_1->GetNbinsX() + 1;
	...
	// Export reco distributions for the new file 
  	...
  	for (Int_t i=0;i!=hdata_obsch_1->GetNbinsX()+1;i++){
    	hdata_bnb_N->SetBinContent(i+1, hdata_obsch_1->GetBinContent(i+1));
		hsignal_N->SetBinContent(i+1, histo_sig_1->GetBinContent(i+1));
		hbackground_N->SetBinContent(i+1, histo_bkg_1->GetBinContent(i+1));
		hextbnb_N->SetBinContent(i+1, histo_ext_1->GetBinContent(i+1));
  	}
  	...
  	for (Int_t i=0;i!=hdata_obsch_1->GetNbinsX()+1;i++){
    	hmeas->SetBinContent(i+1, hdata_obsch_1->GetBinContent(i+1) - histo_bkg_1->GetBinContent(i+1) - histo_ext_1->GetBinContent(i+1));
