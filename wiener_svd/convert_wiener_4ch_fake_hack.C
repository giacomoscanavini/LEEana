/*
  ...
  To run use: 
    root -l --web=off 'convert_wiener_4ch.C(1)'   (yes reweighting)
    root -l --web=off 'convert_wiener_4ch.C(0)'   (no reweighting)
*/
void convert_wiener_4ch_fake_hack(int RW=1){

  bool LIMITED_SYS = true;  // true excludes DET, FLUX, GEANT4 systematics from cov
  ...
  //TH1F *histo_ext_1 = (TH1F*)file1->Get("histo_9");  // EXT FC 0p
  //TH1F *histo_ext_2 = (TH1F*)file1->Get("histo_10"); // EXT FC Np
  //TH1F *histo_ext_3 = (TH1F*)file1->Get("histo_11"); // EXT PC 0p
  //TH1F *histo_ext_4 = (TH1F*)file1->Get("histo_12"); // EXT PC Np
  ...
  for (Int_t i=0;i!=hdata_obsch_1->GetNbinsX()+1;i++){
    ...
    hmeas->SetBinContent(A, histo_sig_1->GetBinContent(i+1));//hdata_obsch_1->GetBinContent(i+1) - histo_bkg_1->GetBinContent(i+1)); //- histo_ext_1->GetBinContent(i+1));
    hmeas->SetBinContent(B, histo_sig_2->GetBinContent(i+1));//hdata_obsch_2->GetBinContent(i+1) - histo_bkg_2->GetBinContent(i+1)); //- histo_ext_2->GetBinContent(i+1));
    hmeas->SetBinContent(C, histo_sig_3->GetBinContent(i+1));//hdata_obsch_3->GetBinContent(i+1) - histo_bkg_3->GetBinContent(i+1)); //- histo_ext_3->GetBinContent(i+1));
    hmeas->SetBinContent(D, histo_sig_4->GetBinContent(i+1));//hdata_obsch_4->GetBinContent(i+1) - histo_bkg_4->GetBinContent(i+1)); //- histo_ext_4->GetBinContent(i+1));
  ...
  //for (Int_t i=0;i!=histo_ext_3->GetNbinsX()+1;i++){ vec_nominal(noffset + i) = histo_ext_3->GetBinContent(i+1);
  //}
  //noffset += histo_ext_3->GetNbinsX()+1;
  //for (Int_t i=0;i!=histo_ext_4->GetNbinsX()+1;i++){ vec_nominal(noffset + i) = histo_ext_4->GetBinContent(i+1);
  //}
  //noffset += histo_ext_4->GetNbinsX()+1;
  ...
  */
  ...
  if(LIMITED_SYS == false) hcov_tot->Add(hcov_add);
  if(LIMITED_SYS == false) hcov_tot->Add(hcov_flux);    
  //hcov_tot->Add(hcov_xs);
  if(LIMITED_SYS == false) hcov_tot->Add(hcov_geant4); 
  hcov_tot->Add(hcov_genie);
  if(LIMITED_SYS == false) hcov_tot->Add(hcov_det);     
  if(RW==1 && LIMITED_SYS == false) hcov_tot->Add(hcov_rw);  
  if(RW==1 && LIMITED_SYS == false) hcov_tot->Add(hcov_rw_cor);  
  ...
  h20->SetLineColor(8);
  if(LIMITED_SYS == false) h30->Draw("same"); //dirt
  if(LIMITED_SYS == false) h30->SetLineColor(3);
  if(LIMITED_SYS == false) h40->Draw("same"); //flux
  if(LIMITED_SYS == false) h40->SetLineColor(2);
  if(LIMITED_SYS == false) h50->Draw("same"); //det
  if(LIMITED_SYS == false) h50->SetLineColor(6);
  //h60->Draw("same"); // xs
  //h60->SetLineColor(4);
  if(LIMITED_SYS == false) h59->Draw("same"); // geant4
  if(LIMITED_SYS == false) h59->SetLineColor(kBlue-7);
  h60->Draw("same"); // genie
  h60->SetLineColor(4);
  ...
  h10->SetLineWidth(2);
  h20->SetLineWidth(2);
  if(LIMITED_SYS == false) h30->SetLineWidth(2);
  if(LIMITED_SYS == false) h40->SetLineWidth(2);
  if(LIMITED_SYS == false) h50->SetLineWidth(2);
  if(LIMITED_SYS == false) h59->SetLineWidth(2);
  h60->SetLineWidth(2);
  ...
  if(LIMITED_SYS == false) le1->AddEntry(h30,"Dirt","l");
  if(LIMITED_SYS == false) le1->AddEntry(h40,"Flux","l");
  if(LIMITED_SYS == false) le1->AddEntry(h50,"Det.","l");
  //le1->AddEntry(h60,"Xs","l");
  if(LIMITED_SYS == false) le1->AddEntry(h59,"Geant4","l");
